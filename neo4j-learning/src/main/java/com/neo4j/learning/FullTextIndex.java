...
public class FullTextIndex {
    ...
    /**
     * This declares the first of two procedures in this class - a procedure
     * that performs queries in a manual index.
     *
     * It returns a Stream of Records, where records are specified per
     * procedure. This particular procedure returns a stream of
     * {@link SearchHit} records.
     *
     * The arguments to this procedure are annotated with the {@link Name}
     * annotation and define the position, name and type of arguments required
     * to invoke this procedure. There is a limited set of types you can use for
     ...
     * <li>{@link java.util.Map} with key {@link String} and value
     * {@link Object}</li>
     * <li>{@link java.util.List} of elements of any valid argument type,
     * including {@link java.util.List}</li>
     * <li>{@link Object}, meaning any of the valid argument types</li>
     * </ul>
     *
     * @param label the label name to query by
     * @param query the lucene query, for instance `name:Brook*` to search by
     * property `name` and find any value starting with `Brook`. Please refer to
     * the Lucene Query Parser documentation for full available syntax.
    ...
    /**
     * This is the second procedure defined in this class, it is used to update
     * the index with nodes that should be queryable. You can send the same node
     * multiple times, if it already exists in the index the index will be
     * updated to match the current state of the node.
     *
     * This procedure works largely the same as {@link #search(String, String)},
     * with three notable differences. One, it is annotated with `mode =
     * SCHEMA`, which is <i>required</i> if you want to perform updates to the
     * graph in your procedure.
     *
     * Two, it returns {@code void} rather than a stream. This is simply a
     * short-hand for saying our procedure always returns an empty stream of
     * empty records.
     *
     * Three, it uses a default value for the property list, in this way you can
     * call the procedure by simply invoking {@code CALL index(nodeId)}. Default
     * values are are provided as the Cypher string representation of the given
     * type, e.g. {@code {default: true}}, {@code null}, or {@code -1}.
     *
    ...
    /**
     * This is the output record for our search procedure. All procedures that
     * return results return them as a Stream of Records, where the records are
     * defined like this one - customized to fit what the procedure is
     * returning.
     *
     ...
     * <li>{@link java.util.Map} with key {@link String} and value
     * {@link Object}</li>
     * <li>{@link java.util.List} of elements of any valid field type, including
